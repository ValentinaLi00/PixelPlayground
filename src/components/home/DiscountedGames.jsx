        ...
        <div className={classes.container}>
            ...
            <div className={classes.scroller}>
                ...
                <div className={classes.scroller_inner}>
                    {data && data.slice(0, show).map((game) => (<Link to={`/${game.id}`}><DiscountedGame key={game.id} game={game} /></Link>))}
                </div>

                {/* scroller inner 2 */}
                <div className={classes.scroller_inner}>
                    {data && data.slice(show, 2*show).map((game) => (<Link to={`/${game.id}`}><DiscountedGame key={game.id} game={game} /></Link>))}
                </div>
